...
const HoverCard = ({ title, subtitle, bgColor, cardImages, comingSoon = false , redirectUrl}) => {
  const [hovered, setHovered] = useState(false);
  const navigate = useNavigate();

  const handleClick = () => {
    if (!comingSoon && redirectUrl) {
      navigate(redirectUrl); // uses react-router's client-side navigation
    }
  };

  return (
    <div
      className="relative w-full h-[120px] mx-auto overflow-visible my-5 hover:my-14 transition-all duration-300 ease-in-out"
      onMouseEnter={() => !comingSoon && setHovered(true)}
      onMouseLeave={() => !comingSoon && setHovered(false)}
      onClick={handleClick}
    >
      {/* Stacked Pyramid Cards */}
      {!comingSoon &&
  cardImages?.slice(1).map((img, index) => {
    ...
    return (
      <motion.div
        ...
        animate={{
          y: hovered ? 0 : offsetY,
          opacity: hovered ? 0.9 - index * 0.2 : 0,
        }}
        transition={{
          type: "spring",
          stiffness: 200,
      ...
      <motion.div
        ...
        style={{
          backgroundImage: hovered && !comingSoon ? `url(${cardImages[0]})` : "none",
        ...
        initial={{ scale: 1 }}
        animate={{
          scale: hovered && !comingSoon ? 1.03 : 1,
          y: hovered && !comingSoon ? -3 : 0,
        }}
